...
public class AuthorizationEndpoint {
  ...
  public static ResponseEntity<String> handleAuthorizationPost(HttpEntity<String> entity,
    ...
    if (!aud.equals(baseUrl)) {
      ...
      logger.info(
          "AuthorizationEndpoint::handleAuthorizationPost:Test patient and practioner: " + params.get("providerId")
              + " "
              + params.get("practitionerId"));

      String code = AuthUtils.generateAuthorizationCode(baseUrl, clientId, redirectURI, params.get("providerId"),
          params.get("practitionerId"));
